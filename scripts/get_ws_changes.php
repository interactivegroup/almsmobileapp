...
if (!isset($argv[1])) {
    echo "ERROR: Please pass the path to the folder containing the Alms installations as the first parameter.\n";
    die();
}
...
foreach ($versions as $version) {
    $almspath = concatenate_paths($almsspath, 'stable_' . $version, $pathseparator);
    ...
    // Get the structure in this Alms version.
    $structure = shell_exec("php $scriptpath $almspath $wsname " . ($useparams ? 'true' : ''));
